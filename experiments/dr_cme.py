...
def main():
    ...
    # Sample from P and Q
    X_p, Y = sample_joint(
        n_train, sample_covariate_p, conditional_y, seed=seed
    )
    X_q, Z = sample_joint(
        n_train, sample_covariate_q, conditional_z, seed=seed + 1
    )
    ...
    cme_Y = cme_model(X_test, X_p_train, Y_train, lam_p, gaussian_kernel, bandwidth=bandwidth)
    cme_Z = cme_model(X_test, X_q_train, Z_train, lam_q, gaussian_kernel, bandwidth=bandwidth)
