...
def conditional_z(X: np.ndarray, theta: float, noise_std: float = 0.5, seed: int = None) -> np.ndarray:
    """
    ...
    rng = np.random.default_rng(seed)
    n = len(X)
    # mean = np.sin(np.pi * (X - theta))
    mean = (1-theta) * np.sin(np.pi * (X))
    noise = rng.normal(0, noise_std, size=n)
    return mean + noise
...
def sample_joint(
    ...
    Y = conditional_fn(X, theta, noise_std=noise_std, seed=seed_y)
